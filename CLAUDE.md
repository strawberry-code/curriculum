...
1. **Single-file HTML.** `index.html` è autocontenuto: HTML + `<style>` + `<script>` inline. NON spezzare in asset esterni senza approvazione esplicita utente. (Il tool di export PDF in `scripts/` è un progetto locale separato, non tocca l'autocontenimento di `index.html` — vedi punto 10.)
2. **Bilingue IT/EN.** Ogni stringa visibile DEVE avere `data-i18n="key"` + entry nel dict `T_EN` dentro `<script>`. Vedi `docs/I18N.md`.
3. **Design system fisso.** Palette CSS variables, font (Geist / JetBrains Mono), semantica chip — vedi `docs/STYLE_GUIDE.md`. NON introdurre colori/font fuori sistema senza approvazione.
...
9. **Aggiorna data versione.** Ad ogni modifica sostanziale al CV aggiorna la data "ultima modifica" al mese corrente in 3 punti di `index.html`:
   - `nav-clock`: `v. <Mese-abbr-IT> AAAA` (es. `v. Giu 2026`)
   - `foot_meta_1` IT: `CV · v. <Mese-IT> AAAA` (es. `CV · v. Giugno 2026`)
   - `foot_meta_1` nel dict `T_EN`: `CV · v. <Month-EN> AAAA` (es. `CV · v. June 2026`)
10. **PDF generati, mai editati a mano.** `cv-cristiano-cavo-it.pdf` / `-en.pdf` (root) sono prodotti da `npm run build:pdf` (`scripts/build-pdf.mjs`), che legge il DOM live di `index.html` (fonte unica di verità, IT + click su `[data-lang="en"]`). Non modificare mai i PDF direttamente né riscrivere il loro contenuto a mano altrove: se serve un cambiamento, va fatto in `index.html` e poi rigenerato. Setup one-time locale: `npm install && npx playwright install chromium`.
11. **Consistenza web↔PDF obbligatoria.** Se il profilo/contenuto del CV cambia in `index.html` (esperienze, stack, formazione, affiliazioni, contatti, "cosa offro") il task NON è completo finché non hai anche: (a) rilanciato `npm run build:pdf`, (b) verificato che i due PDF riflettano il nuovo contenuto (es. `pdftotext -layout` + grep sulla stringa cambiata, o ispezione visiva pagina per pagina), (c) committato i PDF rigenerati insieme al resto (stesso commit/amend, mai in un passaggio separato successivo). Non basta "ricordarsi di rigenerare quando serve" — è un gate bloccante, non un promemoria opzionale. Se per qualsiasi motivo non rigeneri i PDF in una sessione (es. Playwright non disponibile), dillo esplicitamente all'utente invece di lasciarli silenziosamente disallineati.
...
6. Se il contenuto (esperienze, stack, formazione, affiliazioni, contatti, "cosa offro") è cambiato: `npm run build:pdf` per rigenerare `cv-cristiano-cavo-it.pdf` / `-en.pdf` e verifica che riflettano il nuovo contenuto — vedi principio 11, è un passaggio obbligatorio, non facoltativo.
7. Commit conv. + push. Vercel deploya auto se collegato a GitHub.
